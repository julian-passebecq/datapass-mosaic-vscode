...
| Part | What happens |
| --- | --- |
| Data statements | Really run on the shared local catalog (DuckDB). T-SQL is translated for a documented subset: `[brackets]`, `N'...'`, `@variables`, `dbo` (the warehouse layer), `ISNULL`, `LEN`, `COUNT_BIG`, `IIF`, `EOMONTH`, `GETDATE` and its family (a fixed lab clock), `CAST`/`TRY_CAST`/`CONVERT` with T-SQL types, `DATEADD`, `DATEDIFF`, `DATEPART`, `CHARINDEX`, `TOP` (also in subqueries); `OPTION (...)` hints are dropped. String concatenation with `+` is not translated: use `CONCAT`. Anything else must already be valid DuckDB SQL. |
