...
| Unity Catalog | One catalog, `main`; schemas are the lab's layers plus `ml` (models). Owners, grants from `grants.sql` and groups from `unity_catalog.json`; every read and write of a job running `run_as` a principal is checked (USE CATALOG, USE SCHEMA, SELECT, MODIFY, CREATE TABLE, CREATE MODEL, EXECUTE, ownership). |
...
- `grants.sql`: `GRANT`, `REVOKE` and `ALTER ... OWNER TO` statements, applied before each run.
...
cluster policies, secrets, volumes, row filters and column masks, model serving,
