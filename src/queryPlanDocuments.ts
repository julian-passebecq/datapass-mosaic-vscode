...
import type { DialectTranslationView, QueryPlanView } from "./webview/contracts";
...
/** The DuckDB SQL a dialect file was translated to, read-only beside the Workbench (`<file>.duckdb.sql`). */
export async function openTranslatedSql(translation: DialectTranslationView, source?: string): Promise<void> {
  const label = (source ?? "query").replace(/\.sql$/i, "").replace(/[^A-Za-z0-9_.-]+/g, "-").replace(/^-+|-+$/g, "") || "query";
  const uri = vscode.Uri.from({ scheme: QUERY_PLAN_SCHEME, path: `/${label}.duckdb.sql` });
  plans.set(uri.toString(), [
    `-- ${translation.label}: the DuckDB SQL that ran on the local catalog.`,
    ...translation.rewrites.map(rewrite => `-- Rule: ${rewrite}`),
    "",
    translation.sql,
    ""
  ].join("\n"));
