...
- **Concept checks** (language `quiz`, runtime `datapass-quiz-v1`, `runtime/datapass_runtime/quiz_grading.py`): the learner
  writes `answer: <letter(s) or short text>` in `solution.txt`; a Run checks only that the answer is well formed, a Submit
  compares it with the private answer in `grading.server.json` and returns the explanation only when it is right. Truth
  `concept-check` (new value in the exercise contracts). Pack `concepts-v1`: 24 cards (Fabric capacities, Synapse
  dedicated SQL pool, Databricks compute, Unity Catalog, Delta / Iceberg / DuckLake), generated by
  `scripts/authoring/gen_concepts.py`; each card's `context_refs` names one of 5 reference sheets in `content/reference/`
  (shipped in the VSIX), opened by the card's "Reference" button as a native Markdown preview (the host resolves the path
  from the pack; the webview sends only the card key and an index).
- **pytest exercises** (language `pytest`, runtime `datapass-pytest-v1`, `runtime/datapass_runtime/pytest_grading.py`): the
  learner's `solution.py` holds the function and its tests. Visible check `own-tests`: the learner's tests pass (at least
  3). Hidden check (Submit): the pack's pytest file imports the module as `solution`. Each run is a child
  `python -m pytest` of the worker in a fresh temporary folder (60 s timeout, `PYTEST_DISABLE_PLUGIN_AUTOLOAD`, no cache,
  environment without `DATAPASS_*` or secret-looking variables); refused with "Trusted local Python is off" while trusted
  Python is off. Pack `python-prod-v1`: 8 exercises (parse amounts without float, CSV totals, logging dropped rows,
  config errors with chaining, retry with backoff, lazy batches, latest record per key, atomic safe file writes),
  generated by `scripts/authoring/gen_python_prod.py`. `pytest>=8` added to the runtime dependencies.
- Extension: labels "Concept check" and "Python + pytest", notices on the card, `solution.py` for pytest exercises, the
  reference buttons (`openReference` message, `REFERENCE_SHEET` path allowlist).

Checked: exercise_packs_smoke (the two packs add 32 references passing, 32 starters and 55 mutants rejected); a host-suite
step (concept check graded, Run without the explanation, pytest refused while untrusted); the UI pass grades a concept
check, opens its reference sheet, and grades a pytest exercise with trusted Python on; all CLAUDE.md gates (see the PR).
Not checked: pytest exercises on macOS/Linux desktops beyond CI.
